...
#include <zephyr/bluetooth/hci_vs.h> // Note trying this out muna
...
// ----- for cont mode ----- //
static volatile bool cont_mode = false;
#define CHUNK_SIZE 100  // Each chunk contains 100 samples (200 bytes)
static int16_t cont_buf[CHUNK_SIZE] = {0};
static int cont_buf_index = 0;
...
// ---------------------------------------------------- IMU Implementation -------------------------------------------------------------- //
#define MIC_RECORD_DURATION_SECONDS 20
// Note: changing the seconds recording here will change both IMU and MIC Buffers
/* IMU variables */
#define STACK_SIZE 2048
#define IMU_THREAD_PRIORITY 5
#define MOTION_THRESHOLD 15  // Adjust based on expected motion levels
#define IMU_SAMPLE_RATE_HZ 100
// #define IMU_RECORD_DURATION_SECONDS 10 -- will just get the seconds from the MIC_RECORD_DURATION_SECONDS
#define IMU_SAMPLE_COUNT (IMU_SAMPLE_RATE_HZ * MIC_RECORD_DURATION_SECONDS)
...
static int16_t imu_buf[IMU_SAMPLE_COUNT] = {0}; // assuming we are using 100 Hz sampling rate
...
void imu_thread(void *arg1, void *arg2, void *arg3)
{
    ...
    while (1) {
        ...
        if (IMU_index < IMU_SAMPLE_COUNT && imu_recording == true){
		imu_buf[IMU_index] = (int16_t)(motion_magnitude * 100);  // For 2 decimal precision
		IMU_index++;
	    }
	    else if (IMU_index == IMU_SAMPLE_COUNT){
...
// #define MIC_RECORD_DURATION_SECONDS 10 -- MOVING THIS AT THE VERY TOP
    ...
    while (1) {
        ...
        if (mic_recording && mic_sample_index < MIC_SAMPLE_COUNT) {
            ...
            if (mic_sample_index >= MIC_SAMPLE_COUNT) {
                ...
                // // ✅ Write "finished" after loop
                // const char *end_msg = "finished\n";
                // fs_write(&sd_file, end_msg, strlen(end_msg));

                // fs_close(&sd_file);  // ✅ Close the SD file
            }
        }
        uint32_t t_end = k_cycle_get_32();
        uint32_t elapsed_us = k_cyc_to_us_floor32(t_end - t_start);
        // k_busy_wait(500 - elapsed_us); // might have problem with this because of wait time
        /*
        k_busy_wait() expects an unsigned int, so passing a negative value like -50 turns into
        a very large unsigned value (e.g., 4294967246 if using 32-bit), resulting in a very long
        delay—likely several seconds or even more.
        */
        uint32_t wait_time = (elapsed_us < 500) ? (500 - elapsed_us) : 0;
...
static void stream_imu_data()
{
    ...
    // Footer message
    const char *footer_msg = "IMU\n";
    bt_nus_send(NULL, footer_msg, strlen(footer_msg));

    // Motion status footer -- removed flagging because we already passing the whole graph recorded
    // if (motion_flag == 1){
...
static void received(struct bt_conn *conn, const void *data, uint16_t len, void *ctx)
{
    ...
    // Send stored recording when "send" command is received
        ...
        if (cont_mode){
            cont_mode=false;
            printk("Changing cont mode to false and sending remaining chunks and footer");
            // Send any remaining samples (if less than CHUNK_SIZE)
            if (cont_buf_index > 0) {
                bt_nus_send(NULL, cont_buf, cont_buf_index * sizeof(int16_t));
            }
            // send footer message
            const char *footer_msg = "finished\n";
            bt_nus_send(NULL, footer_msg, strlen(footer_msg));
...
int main(void)
{
    ...
    // // Set TX power to +8 dBm for advertising
    // int tx_err = set_tx_power(BT_HCI_VS_LL_HANDLE_TYPE_ADV, 0, 8);
    // if (tx_err) {
    //     LOG_WRN("TX power set failed: %d", tx_err);
    // } else {
    ...
    /* Portion for the IMU computation */ // running at priority level 5
    start_imu_thread();
    /* Portion for the MIC computation */ // running at priority level 5
    start_mic_thread();
            ...
            case MODE_AUTO:
                ...
                while (current_mode == MODE_AUTO) {
                    if (!mic_recording){
                        ...
                        k_sleep(K_SECONDS(MIC_RECORD_DURATION_SECONDS + 2));
                        // send after doing PDM transfer ---------- //
                        stream_audio();    // Send after recording
                        printk("before 5 seconds");
                        k_sleep(K_SECONDS(5)); // we can configure this later on para sa frequency ng pagkuha ng heart rate 
                        printk("after 5 seconds");
                        mic_recording = false;  // <-- Add this line
                        mic_sample_index = 0;
                        IMU_index = 0;
                        printk("%d\n", mic_recording);
                    }
                    k_yield(); // Let others run
                }
...
1.1: Changed mic thread priority to 4 -- trying to remove the humming in the recording (04-29-2025)
...
1.2: Will be changing the recording duration to 20 Seconds and Adding the "cont" mode (05-01-2025)
 - might ignore IMU data in cont mode,,, this is so that we can process in sending chunk
   - thinking we might not implement cont yet because initial implementations didn't work aswell, along with difficulty of having the IMU plotted
     against it.
   - the effectivity of the algorithm works bests for windowed samples, so having longer audio recording will just leverage on the fact that
     we still have to divide the long audio recording (i.e 1 hour recording) into samples of small recordings
 - NOTE: Cont mode is not working but we can record na in 20 Seconds durations for auto and manual
 - made this so that you'll only change MIC_RECORD_DURATION_SECONDS and it will automatically adapt the buffer size for both IMU and MIC
 - moved the elapsed computation to before the very first if statement in the while loop of the Mic Thread
   - hoping this would remove the humming but still it didnt... 
 - also removed the else statement and configured the LED indicators to work as expected:
   - RED == recording in progress
   - BLUE == Done recording, ready to be sent
   - GREEN == Sending buffers
   - OFF == usually idle or just after sending this will be the light mode
   - WHITE == 'cont' mode, but this is for debate whether to continue or not...

1.3: Changed yung sa part ng IMU (05-07-2025)
 - initially hardcoded kasi yung part na if IMU == 1000 then IMU_recording false

1.4: Fixed yung sa part ng SD card but havent enabled yet...

2.0: Made the TX power into the maximum the device could be which is 8 dBm
 - CONFIG_BT_CTLR_TX_PWR_ANTENNA=8
 - Bugs fixed: auto sending, see notes with electret microphone thread implementation
 - "k_busy_wait() expects an unsigned int, so passing a negative value like -50 turns
    into a very large unsigned value (e.g., 4294967246 if using 32-bit), resulting in a
    very long delay—likely several seconds or even more.""
